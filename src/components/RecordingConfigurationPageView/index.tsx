...
const RecordingConfigurationPageView = ({ href, countdown, duration, onDuration, onCountdown }: RecordingConfigurationPageViewProps) =>
    <Pane padding={majorScale(2)}>
        <Pane display="grid" gridTemplateColumns="auto auto" gap={majorScale(2)} alignItems="baseline">
            ...
            <TextInput width="auto" value={countdown} onChange={(e: ETV<string>) => onCountdown(parseFloat(e.target.value))} type="number" min="0"></TextInput>
            <Heading size={400}>Duration: </Heading>
            <TextInput width="auto" value={duration} onChange={(e: ETV<string>) => onDuration(parseFloat(e.target.value))} type="number" min="0" ></TextInput>
